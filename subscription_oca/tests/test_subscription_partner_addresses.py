...
class TestSubscriptionPartnerAddresses(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.customer = cls.env["res.partner"].create({"name": "Main customer"})
    ...
    def test_addresses_default_from_partner_children(self):
        ...
        self.assertEqual(sub.partner_invoice_id, self.invoice_address)
        self.assertEqual(sub.partner_shipping_id, self.shipping_address)
    ...
    def test_invoice_uses_subscription_addresses(self):
        ...
        self.assertEqual(invoice.partner_id, self.invoice_address)
        self.assertEqual(invoice.partner_shipping_id, self.shipping_address)
    ...
    def test_invoice_commercial_partner_rolls_up_to_customer(self):
        ...
        sub = self._new_subscription(self.customer)
        invoice = sub.create_invoice()
        self.assertEqual(invoice.commercial_partner_id, self.customer)
    ...
    def test_sale_order_carries_addresses(self):
        ...
        self.assertEqual(order.partner_invoice_id, self.invoice_address)
        self.assertEqual(order.partner_shipping_id, self.shipping_address)
